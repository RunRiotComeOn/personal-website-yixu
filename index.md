---
layout: default
title: Home
---

<div class="hero-canvas">
  <div class="hero-two-column">
    <!-- Left Column: Profile Image -->
    <div class="hero-image-column">
      {% if site.profile_image %}
        <img src="{{ site.profile_image | relative_url }}" alt="{{ site.author }}" class="profile-image">
      {% else %}
        <div class="profile-placeholder">
          <span>Add your photo at<br>assets/images/profile.jpg</span>
        </div>
      {% endif %}

      <!-- Contact Quick Links -->
      <div class="profile-contact-links">
        {% if site.email %}
          <a href="mailto:{{ site.email }}">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>
            Email
          </a>
        {% endif %}

        {% if site.github %}
          <a href="https://github.com/{{ site.github }}" target="_blank" rel="noopener">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22"></path></svg>
            GitHub
          </a>
        {% endif %}

        {% if site.google_scholar %}
          <a href="https://scholar.google.com/citations?user={{ site.google_scholar }}" target="_blank" rel="noopener">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M12 24a7 7 0 1 1 0-14 7 7 0 0 1 0 14zm0-24L0 9.5l4.838 3.94A8 8 0 0 1 12 9a8 8 0 0 1 7.162 4.44L24 9.5z"/></svg>
            Scholar
          </a>
        {% endif %}

        {% if site.linkedin %}
          <a href="https://linkedin.com/in/{{ site.linkedin }}" target="_blank" rel="noopener">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"></path><rect x="2" y="9" width="4" height="12"></rect><circle cx="4" cy="4" r="2"></circle></svg>
            LinkedIn
          </a>
        {% endif %}
      </div>
    </div>

    <!-- Right Column: Bio Content -->
    <div class="hero-content-column">
      <h1 class="hero-title">{{ site.author }}</h1>
      <p class="hero-tagline">{{ site.description }}</p>

      <div class="bio-extended">
        {{ site.bio_extended | markdownify }}
      </div>
    </div>
  </div>
</div>

<hr class="decorative-divider">

<section class="home-section">
  <h2>Current Research Focus</h2>
  <div class="research-preview">
    {% assign latest_research = site.data.research | first %}
    <h3>{{ latest_research.position }}</h3>
    <p class="institution">{{ latest_research.institution }} &mdash; {{ latest_research.group | markdownify | remove: '<p>' | remove: '</p>' | strip }}</p>
    <p class="dates">{{ latest_research.start_year }} &ndash; {{ latest_research.end_year }}</p>
    <div class="research-description">
      {{ latest_research.description | markdownify }}
    </div>
  </div>

  <p class="view-all">
    <a href="{{ '/research/' | relative_url }}">View all research experience &rarr;</a>
  </p>
</section>
